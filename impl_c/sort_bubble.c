 ...
 * Fall 2015
 * Extra Credit - C implementation of bubblesort
 */
...
void bubble_sort(uint32_t* data, uint32_t size);
...
int main(int argc, char** argv) {
	...
	bubble_sort(active_list, active_list_size);
...
void bubble_sort(uint32_t* data, uint32_t size) {
	uint32_t flip_flag = 1; // 4-byte integers align nicely with the CPU cache.

	while (flip_flag) {
		flip_flag = 0;

		for (uint32_t i = 0; i < size - 1; ++i) {
			uint32_t j = i + 1; // Save some operations by immediately storing the increment.
